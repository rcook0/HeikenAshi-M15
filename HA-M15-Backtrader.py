...
import argparse import datetime as dt import math import backtrader as bt
...
class HeikinAshiSmooth(bt.Indicator): '''Heikin Ashi with optional EMA smoothing on HA close.''' lines = ('ha_open', 'ha_high', 'ha_low', 'ha_close') params = dict(smooth_len=3)
...
class VWAPSlope(bt.Indicator): '''Session-cumulative VWAP with simple slope (vwap - vwap[-1]).''' lines = ('vwap', 'slope',)
...
class StochKD(bt.Indicator): lines = ('k', 'd') params = dict(k=14, d=3, smooth=3) def init(self): ll = bt.ind.Lowest(self.data.low, period=self.p.k) hh = bt.ind.Highest(self.data.high, period=self.p.k) k_raw = bt.If(hh - ll == 0, 0, (self.data.close - ll) / (hh - ll) * 100.0) k_s = bt.ind.EMA(k_raw, period=self.p.smooth) self.l.k = k_s self.l.d = bt.ind.EMA(self.l.k, period=self.p.d)
...
class HAOverlayStrategy(bt.Strategy): params = dict( # General accuracy=0.70,                 # relaxed accuracy → widens thresholds risk_pct=0.10,                 # percent of equity (used to size via sizer) # Heikin Ashi ha_smooth_len=3, # Indicators ema_fast=21, ema_slow=200, atr_len=14, stoch_k=14, stoch_d=3, stoch_smooth=3, vwap_confluence=True, volume_filter=True, vol_ma_len=20, # Sessions (exchange/local tz assumed in data) session_enable=True, london_start=dt.time(7, 0), london_end=dt.time(12, 0), ny_start=dt.time(13, 0), ny_end=dt.time(17, 0), # ATR management atr_stop=1.0,                  # SL = ATR * this * multiplier atr_target=2.0,                # TP = ATR * this * multiplier vol_sensitivity=1.0,           # adaptive ATR sensitivity manual_atr=False, manual_atr_mult=1.0, # Break-even & trailing use_be=True, be_R=1.0, be_offset_atr=0.1, use_trail=True, trail_atr_mult=1.5, )
...
class RiskSizer(bt.Sizer): params = dict(risk_pct=0.10) def _getsizing(self, comminfo, cash, data, isbuy): # size = (equity * risk_pct) / price → naive fixed % of equity position sizing price = data.close[0] target_value = cash * self.p.risk_pct size = max(1, int(target_value / max(price, 1e-9))) return size

class GenericCSV(bt.feeds.GenericCSVData): params = ( ('dtformat', '%Y-%m-%d %H:%M:%S'), ('datetime', 0), ('open', 1), ('high', 2), ('low', 3), ('close', 4), ('volume', 5), ('openinterest', -1), ('timeframe', bt.TimeFrame.Minutes), ('compression', 15), )

def run(args): cerebro = bt.Cerebro() data = GenericCSV(dataname=args.csv, timeframe=bt.TimeFrame.Minutes, compression=int(args.timeframe)) data._name = args.symbol cerebro.adddata(data) cerebro.addsizer(RiskSizer, risk_pct=args.risk)
...
if name == 'main': p = argparse.ArgumentParser(description='HA Overlay Strategy (Backtrader)') p.add_argument('--csv', required=True, help='Path to CSV with datetime,open,high,low,close,volume') p.add_argument('--symbol', default='XAUUSD') p.add_argument('--timeframe', default='15') p.add_argument('--cash', type=float, default=10000) p.add_argument('--comm', type=float, default=0.0002, help='Commission as fraction (e.g., 0.0002 = 0.02%)') p.add_argument('--risk', type=float, default=0.10, help='Position size as %% of equity') p.add_argument('--accuracy', type=float, default=0.70) p.add_argument('--ha-smooth', type=int, default=3) p.add_argument('--ema-fast', type=int, default=21) p.add_argument('--ema-slow', type=int, default=200) p.add_argument('--atr-len', type=int, default=14) p.add_argument('--stoch-k', type=int, default=14) p.add_argument('--stoch-d', type=int, default=3) p.add_argument('--stoch-smooth', type=int, default=3) p.add_argument('--vol-ma', type=int, default=20) p.add_argument('--atr-stop', type=float, default=1.0) p.add_argument('--atr-target', type=float, default=2.0) p.add_argument('--vol-sens', type=float, default=1.0) p.add_argument('--manual-atr', action='store_true') p.add_argument('--manual-atr-mult', type=float, default=1.0) p.add_argument('--no-vol-filter', action='store_true') p.add_argument('--no-sessions', action='store_true') p.add_argument('--no-be', action='store_true') p.add_argument('--be-R', type=float, default=1.0) p.add_argument('--be-off', type=float, default=0.1) p.add_argument('--no-trail', action='store_true') p.add_argument('--trail-atr', type=float, default=1.5) p.add_argument('--plot', action='store_true') args = p.parse_args() run(args)
